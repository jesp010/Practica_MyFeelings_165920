package solis.juan.myfeelings

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.util.Log
import android.widget.Toast
import solis.juan.myfeelings.utilities.CustomBarDrawable
import solis.juan.myfeelings.utilities.CustomCircleDrawable
import solis.juan.myfeelings.utilities.Emociones
import solis.juan.myfeelings.utilities.JSONFile
...
class MainActivity : AppCompatActivity() {
    ...
    fun iconoMayoria() {
        ...
        if (happy > veryHappy && happy > neutral && happy > sad && happy > verySad) {
            icon.setImageDrawable(getDrawable(R.drawable.ic_happy))
        }

        if (veryHappy > happy && veryHappy > neutral && veryHappy > sad && veryHappy > verySad) {
            icon.setImageDrawable(getDrawable(R.drawable.ic_veryhappy))
        }

        if (neutral > veryHappy && neutral > happy && neutral > sad && neutral > verySad) {
            icon.setImageDrawable(getDrawable(R.drawable.ic_neutral))
        }

        if (sad > veryHappy && sad > neutral && sad > happy && sad > verySad) {
            icon.setImageDrawable(getDrawable(R.drawable.ic_sad))
        }

        if (verySad > veryHappy && verySad > neutral && verySad > sad && verySad > happy) {
            icon.setImageDrawable(getDrawable(R.drawable.ic_verysad))
        }
    }
